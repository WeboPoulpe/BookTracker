...
/**
 * Recherche de livre sur deux catalogues complémentaires.
 *
 * Aucun des deux ne suffit seul :
 *   · Open Library a les pages, les couvertures et les genres, mais son
 *     catalogue francophone est troué (§11) ;
 *   · la BnF a tout le dépôt légal français, mais ses notices Dublin Core
 *     n'ont ni pagination ni couverture.
 *
 * On interroge les deux en parallèle et on fusionne. Si l'un tombe, l'autre
 * répond quand même : c'est précisément ce qui manquait quand « Open Library
 * ne répond pas » vidait tout l'écran.
 */
...
export async function rechercherLivre(
  ...
  const [ol, bnf] = await Promise.allSettled([
  ...
  const sources: Record<Source, EtatSource> = {
    openlibrary:
      ol.status === "rejected" ? "echec" : ol.value.length ? "ok" : "vide",
    bnf: bnf.status === "rejected" ? "echec" : bnf.value.length ? "ok" : "vide",
  };
  ...
  const brut: Classe[] = [
    ...(ol.status === "fulfilled" ? ol.value : []).map((r, rang) => ({
      ...r,
      rang,
    })),
    ...(bnf.status === "fulfilled" ? bnf.value : []).map((r, rang) => ({
      ...r,
      rang,
    })),
  ];
  ...
  return {
    ...
    sources,
    toutesEnEchec:
      sources.openlibrary === "echec" && sources.bnf === "echec",
  };
}
